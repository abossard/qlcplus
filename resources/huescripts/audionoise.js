...
(function() {
    ...
    algo.properties.push("name:stretch|type:float|values:0.5,1.5|display:Stretch|write:setStretch|read:getStretch");
    algo.properties.push("name:zoom|type:float|values:0.5,20|display:Zoom|write:setZoom|read:getZoom");
    ...
    algo.setStretch = function(v) { algo.presetStretch = Math.max(0.5, Math.min(1.5, parseFloat(v) || 0.5)); };
    algo.getStretch = function() { return algo.presetStretch; };
    algo.setZoom = function(v) { algo.presetZoom = Math.max(0.5, Math.min(20, parseFloat(v) || 0.5)); };
    ...
    algo.rgbMap = function(width, height, rgb, step, audio) {
        ...
        for (var y = 0; y < height; y++) {
            for (var x = 0; x < width; x++) {
                ...
                var n = HSVUtil.simplex3d(nx, ny, state.z);
                var stretched = n * algo.presetStretch;
                var t = HSVUtil.clamp01((stretched + 1) * 0.5);
                var color = HSVUtil.gradientLedfxAt(algo.colors || [{h: 0.62, s: 1, v: 1}], t);
                var i = (y * width + x) * 3;
                map[i] = color.h;
                map[i + 1] = color.s;
                map[i + 2] = color.v;
