    ...
    // ── Prompt 1: design_dj_show ──────────────────────────────────────
    {
        ...
        p.generator = [doc](const Json &args) -> std::vector<PromptMessage> {
            ...
            std::string guide =
                ...
                "---\n\n"
                + std::string(movingHeadCount > 0 ?
                "## Moving Head Motor Speed Constraints\n\n"
                "Moving heads have physical speed limits. Programming movements faster than the motor "
                "can achieve results in jerky, incomplete motion — the head never reaches position.\n\n"
                "### Typical Speed Reference\n"
                "| Axis | Typical Range | Typical Speed | Rule of Thumb |\n"
                "|------|--------------|---------------|---------------|\n"
                "| Pan | 540° | ~200-250°/s | 1° ≈ 4-5ms |\n"
                "| Tilt | 190° | ~120-150°/s | 1° ≈ 7-8ms (**slower — this is the bottleneck**) |\n\n"
                "### Calculating Minimum Beat Duration for a Move\n"
                "```\n"
                "travel_degrees / tilt_speed_deg_per_sec = seconds_needed\n"
                "seconds_needed × (BPM / 60) = beats_needed\n"
                "Example: 100° tilt @ 130°/s = 0.77s = 1.5 beats @ 120 BPM\n"
                "```\n\n"
                "### Rules for Position Programming\n"
                "1. **Always calculate based on tilt speed** — it's slower than pan in °/s\n"
                "2. **EFX size must scale with speed**: fast cycle = small pattern\n"
                "   - 0.5s cycle → max ~50 width/height (~38° travel)\n"
                "   - 1s cycle → max ~80 width/height (~60° travel)\n"
                "   - 2s cycle → max ~130 width/height (~100° travel)\n"
                "   - 4s+ cycle → any size OK\n"
                "3. **Chaser position steps**: minimum hold ≥ travel_degrees / 130 seconds\n"
                "4. **Random position chasers**: use 2+ beat hold for worst-case jumps\n"
                "5. **Pan/Tilt speed channel** (if available): set to 0 for maximum motor speed\n"
                "6. **Phantom sweeps**: head moves during blackout, beam appears mid-travel — "
                "hides motor lag for dramatic reveals\n\n"
                "---\n\n" : "") +
                "## VC Page Layout\n"
                "```\n"
                "Page \"DJ Control\":\n"
                ...
                + std::string(movingHeadCount > 0 ?
                    "- **Position chasers**: hold time must exceed motor travel time (see Motor Speed Constraints above)\n"
                    "- **EFX patterns**: size must fit within cycle time — fast EFX = small pattern\n" : "") + "\n"
