    ...
    // ── Prompt 1: design_dj_show ──────────────────────────────────────
    {
        ...
        p.generator = [doc](const Json &args) -> std::vector<PromptMessage> {
            ...
            std::string guide =
                ...
                "3. **Create palettes first** — reusable color, position, and dimmer definitions:\n"
                "   - `create_palettes` with type Dimmer, Color, Pan, Tilt, or PanTilt\n"
                "   - Use fanning for gradients across fixtures (Linear, Sine, Square, Saw)\n"
                "4. Build scenes from palettes — combine fixtures + palette references:\n"
                "   - `create_scenes` with `fixtureNames` + `paletteNames` (not raw channelValues)\n"
                "   - Use `channelValues` only for fine-tuning overrides on top of palettes\n"
                "5. Create beat-synced chasers (always use `tempoType: \"beats\"`)\n"
                "6. Bundle into phase collections (P1-P4)\n"
                "7. Build VC page with `vc_create_widgets` + `vc_reflow_frame`\n\n"
                ...
                "- **Palette-first**: Create palettes, then reference them in scenes\n"
                "  - Color palettes: \"Deep Blue\", \"Warm Amber\", \"Magenta Flash\"\n"
                "  - Position palettes: \"Center Stage\", \"Wide Sweep\", \"Audience Wash\"\n"
                "  - Dimmer palettes: \"Full\", \"Half\", \"Blackout\"\n"
                "- **Mood scenes**: fixtureNames + paletteNames for color + dimmer palettes\n"
                "- **Position scenes**: fixtureNames + paletteNames for position palettes\n"
                "- **Energy scenes**: fixtureNames + channelValues for speed/gobo (no palettes needed)\n"
                "- **NEVER mix** layers in one scene\n"
                "- Use `channelValues` only for overrides on top of palettes\n"
                "- Use fanning on palettes for gradient effects across fixtures\n\n"
                ...
                "  - Supports `channelFilter` (dimmer, color, position, gobo, shutter, beam)\n"
                "  - `merge: true` keeps existing scene values, `false` replaces all\n"
                "  - Great for \"save what I see now\" workflow\n\n"

                "## Non-Negotiable Rules\n"
                "1. **Palette-first design**: Build palettes → reference in scenes → override only when needed\n"
                "2. **Palette discipline**: 80%+ runtime inside phase palette\n"
                "3. **Contrast rhythm**: Every phase needs rest + hit state\n"
                "4. **Strobe restraint**: Flash-button accents only, never sustained\n"
                "5. **Audio-reactivity**: Every phase has at least one reactive element\n"
                "6. **Layer separation**: Same channel NEVER in two layers\n"
                "7. **Beat-sync default**: All chasers use tempoType \"beats\"\n"
                "8. **Naming**: Phase-prefix (P1-, P2-) or layer-prefix (Mood-, FX-)\n"
                "9. **Idempotent tools**: All create/add tools upsert by name — safe to call repeatedly\n"
                "10. **Fanning**: Use palette fanning for gradients (color washes, position spreads)\n";
