...
/*********************************************************************
 * VDJ Song Manager integration
 *
 * Connected from App once the VdjBridge exists. The auto-create
 * pipeline is gated on autoCreateSongs (default off) so projects that
 * don't want VDJ-driven show management see exactly the previous
 * behaviour.
 *
 * Per OS2L spec the song event delivers metadata (name, artist, bpm,
 * elapsed, duration, ...) but NOT the audio file path. We treat the
 * 'path' field as optional: when populated by a future richer bridge
 * (e.g. a DMXDesktop-protocol backend) we additionally attach an
 * Audio function — which gives the user a waveform on the timeline
 * for free via the existing WaveformImageProvider. Without a path we
 * still create a BPM-locked Show, which renders the beat grid.
 *********************************************************************/

void ShowManager::setAutoCreateSongs(bool enable)
{
    if (enable == m_autoCreateSongs)
        return;
    m_autoCreateSongs = enable;
    // Reset the de-dup key so toggling on after a song was already
    // received still creates/opens the show on the next event.
    m_lastAutoCreatedSongKey.clear();
    emit autoCreateSongsChanged();
}

static QString vdjSongKey(const QVariantMap &song)
{
    const QString path = song.value("path").toString();
    if (!path.isEmpty())
        return QStringLiteral("path:") + path;
    const QString artist = song.value("artist").toString();
    const QString name = song.value("name").toString();
    if (!artist.isEmpty())
        return QStringLiteral("name:") + artist + QStringLiteral(" - ") + name;
    return QStringLiteral("name:") + name;
}

static QString vdjShowName(const QVariantMap &song)
{
    const QString artist = song.value("artist").toString();
    const QString name = song.value("name").toString();
    if (!artist.isEmpty() && !name.isEmpty())
        return artist + QStringLiteral(" - ") + name;
    if (!name.isEmpty())
        return name;
    return ShowManager::tr("Unknown song");
}

void ShowManager::slotVdjSongChanged()
{
    if (!m_autoCreateSongs)
        return;
    QObject *bridge = sender();
    if (bridge == nullptr)
        return;
    const QVariant v = bridge->property("currentSong");
    if (!v.isValid())
        return;
    ensureShowForVdjSong(v.toMap());
}

int ShowManager::ensureShowForVdjSong(QVariantMap song)
{
    const QString name = song.value("name").toString();
    if (name.isEmpty())
        return Function::invalidId();

    const QString key = vdjSongKey(song);
    if (key == m_lastAutoCreatedSongKey && m_currentShow != nullptr)
        return m_currentShow->id();

    const QString showName = vdjShowName(song);
    const int bpm = qRound(song.value("bpm").toDouble());

    // Look up by name first — re-using existing Shows is the whole point of
    // making VDJ-driven creation idempotent across sessions.
    Show *show = nullptr;
    for (Function *f : m_doc->functionsByType(Function::ShowType))
    {
        if (f->name() == showName)
        {
            show = qobject_cast<Show *>(f);
            break;
        }
    }
    ...
    if (show == nullptr)
    {
        ...
        if (m_doc->addFunction(show) == false)
        {
            qWarning() << "[ShowManager] Failed to add auto-created show for" << showName;
            delete show;
            return Function::invalidId();
        }
        created = true;
        Tardis::instance()->enqueueAction(
            Tardis::FunctionCreate, show->id(), QVariant(),
            Tardis::instance()->actionToByteArray(Tardis::FunctionCreate, show->id()));
    }
    else if (bpm > 0 && show->timeDivisionBPM() != bpm)
    {
        // Existing show — only refresh BPM if VDJ has a value we'd otherwise
        // miss. Don't downgrade the user's choice of time division (Time vs BPM).
        show->setTimeDivisionBPM(bpm);
    }

    // Optional: if VDJ supplied an audio file path that's actually readable,
    // attach an Audio function to track 0 so the timeline draws a waveform.
    // Stock OS2L never carries a path; only a future richer bridge does.
    const QString path = song.value("path").toString();
    if (!path.isEmpty())
    {
        QFileInfo fi(path);
        if (fi.exists() && fi.isReadable())
        {
            ...
            for (Track *t : show->tracks())
            {
                for (ShowFunction *sf : t->showFunctions())
                {
                    ...
                    Audio *a = qobject_cast<Audio *>(f);
                    if (a && a->getSourceFileName() == path)
                    {
            ...
            if (!alreadyAttached)
            {
                ...
                if (m_doc->addFunction(audio))
                {
                    ...
                    audio->setTempoType(Function::Time);
                    sf->setStartTime(0);
                    sf->setDuration(audio->totalDuration() ? audio->totalDuration()
                                                           : qRound(song.value("duration").toDouble() * 1000.0));
        ...
        {
            qDebug() << "[ShowManager] VDJ song path not readable:" << path;
        }
    }

    setCurrentShowID(show->id());
    m_lastAutoCreatedSongKey = key;
    emit vdjSongShowResolved(show->id(), created);
    return show->id();
}
